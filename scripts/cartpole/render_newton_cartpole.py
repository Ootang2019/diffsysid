...
def replay_from_result(result_path: Path, variant: str):
    ...
    model = make_model(
        init_cart_pos=float(q[0, 0]),
        init_pole_angle=float(q[0, 1]),
        init_cart_vel=float(qd[0, 0]),
        init_pole_angvel=float(qd[0, 1]),
        requires_grad=False,
    )
...
def main():
    ...
    p.add_argument("--init-cart-pos", type=float)
    p.add_argument("--init-pole-angle", type=float)
    p.add_argument("--init-cart-vel", type=float)
    p.add_argument("--init-pole-angvel", type=float)
    ...
    p.add_argument("--cam-y", type=float, default=-2.75)
    p.add_argument("--cam-z", type=float, default=1.0)
    p.add_argument("--cam-pitch", type=float, default=-8.0)
    ...
    if args.result_json is not None:
        ...
        required = [args.init_cart_pos, args.init_pole_angle, args.init_cart_vel, args.init_pole_angvel, args.steps, args.dt]
        if any(v is None for v in required):
            raise SystemExit("Numeric rollout requires --init-cart-pos --init-pole-angle --init-cart-vel --init-pole-angvel --steps --dt")

...
def default_result_json(root: Path) -> Path:
    path = root / "outputs" / "slide_cube" / "sysid" / "result.json"
    if path.exists():
        return path
    raise FileNotFoundError(f"No slide_cube result json found at {path}")


def main(argv=None):
    argv = resolve_config_argv(argv, script_file=__file__, default_config_name="render_cfg.json", config_section="single", positional_keys=("result_json",))
    p = argparse.ArgumentParser(description="Generate slide_cube summary artifacts and gt/init/fit compare renders beside a sysID result.")

...
def display_value(raw_value: float, *, fit_param: str, angle_mode: str) -> float:
    if angle_mode == "top_offset" and fit_param == "init_angle":
...
def run(args):
    gt_value_raw = top_offset_to_raw_angle(args.gt_value) if args.angle_mode == "top_offset" and args.fit_param == "init_angle" else args.gt_value
    ...
    result = {
        ...
        "initial_guess": {args.fit_param: display_value(init_param, fit_param=args.fit_param, angle_mode=args.angle_mode), f"{args.fit_param}_raw": init_param, "loss": float(initial_losses[initial_best_idx]), "rmse": float(initial_rmses[initial_best_idx]), "env_index": initial_best_idx},
        ...
        "final_fit": {args.fit_param: display_value(fit_param, fit_param=args.fit_param, angle_mode=args.angle_mode), f"{args.fit_param}_raw": fit_param, "loss": float(best["metrics"]["loss"]), "rmse": float(best["metrics"]["rmse"]), "grad": float(best["metrics"]["grad"]), "best_env_index": best["env_index"], "best_iteration": best["iteration"]},

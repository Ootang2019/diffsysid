...
def run(args):
    ...
    init_angle_raw = parameter_to_raw("init_angle", args.init_angle, args.angle_mode)
    ...
    for name, value in fixed.items():
        ...
        fixed_display[name] = display_param(name, value, args.angle_mode)
    ...
    def pack_param_section(raw_values: np.ndarray, *, include_metrics: dict | None = None, include_selection: str | None = None):
        ...
        for name, raw_value in zip(fit_params, raw_values):
            payload[name] = display_param(name, raw_value, args.angle_mode)

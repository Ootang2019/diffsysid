...
def run(args):
    ...
    def display_param(name: str, raw_value: float) -> float:
        return display_angle(raw_value) if name == "init_angle" else raw_value
    ...
    result = {
        ...
        "loss_observable": "world trajectory of the pendulum tip point (local point [0,0,-1] on the link body)",
        "fit_param": args.fit_param,
        "angle_convention": {
            "mode": args.angle_mode,
            "raw_zero_meaning": "pendulum hanging straight down",
        ...
        "ground_truth": {args.fit_param: display_param(args.fit_param, gt_value_raw), f"{args.fit_param}_raw": gt_value_raw},
        "fixed_parameters": fixed_display,
        "initial_guess": {args.fit_param: display_param(args.fit_param, init_value_raw), f"{args.fit_param}_raw": init_value_raw, "loss": initial_metrics["loss"], "rmse": initial_metrics["rmse"]},
        "final_fit": {args.fit_param: display_param(args.fit_param, final_param_value), f"{args.fit_param}_raw": final_param_value, "loss": final_metrics["loss"], "rmse": final_metrics["rmse"], "grad": final_metrics["grad"], "selection": selection},
        "best_seen": {args.fit_param: display_param(args.fit_param, best["param_value"]), f"{args.fit_param}_raw": best["param_value"], "loss": best["loss"], "rmse": best["rmse"]},
        "config": vars(args),
        "replay": {

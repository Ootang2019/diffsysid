...
def replay_from_result(result_path: Path, variant: str):
    ...
    model = make_model(init_angle=float(q[0, 0]), init_angvel=float(qd[0, 0]), requires_grad=False)
    return data, model, q, qd, time


def numeric_rollout(args):
    model = make_model(init_angle=args.init_angle, init_angvel=args.init_angvel, requires_grad=False)
    solver = newton.solvers.SolverFeatherstone(model, angular_damping=0.0)
    s0 = model.state()
    s1 = model.state()
    control = model.control()
    q = np.empty((args.steps + 1, 1), dtype=np.float32)
    qd = np.empty((args.steps + 1, 1), dtype=np.float32)
...
def main():
    ...
    p.add_argument("--init-angle", type=float)
    p.add_argument("--init-angvel", type=float)
    ...
    p.add_argument("--cam-y", type=float, default=-3.75)
    p.add_argument("--cam-z", type=float, default=1.35)
    ...
    if args.result_json is not None:
        ...
        required = [args.init_angle, args.init_angvel, args.steps, args.dt]
        if any(v is None for v in required):
            raise SystemExit("Numeric rollout requires --init-angle --init-angvel --steps --dt")

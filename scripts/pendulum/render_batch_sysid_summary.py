...
def default_result_json(root: Path) -> Path:
    path = root / "outputs" / "newton_pendulum_batch_sysid" / "result.json"
    if path.exists():
        return path
    raise FileNotFoundError(f"No pendulum batch result json found at {path}")
...
def main(argv=None):
    ...
    p = argparse.ArgumentParser(description="Generate pendulum batch summary artifacts and gt/init/fit compare renders beside a batch sysID result.")
    p.add_argument("result_json", type=Path, nargs="?")
    p.add_argument("--output-dir", type=Path)
    p.add_argument("--label", type=str, default="pendulum_batch_compare")
    ...
    run(
        [
            str(python),
            str(Path(__file__).with_name("render_compare_pendulum.py")),
            "--result-json",
            str(result_json),
            "--output-dir",

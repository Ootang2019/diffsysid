...
def default_result_json(root: Path) -> Path:
    path = root / "outputs" / "newton_pendulum_sysid" / "result.json"
    if path.exists():
        return path
    raise FileNotFoundError(f"No pendulum result json found at {path}")
...
def main(argv=None):
    ...
    p = argparse.ArgumentParser(description="Generate pendulum summary artifacts and gt/init/fit compare renders beside a sysID result.")

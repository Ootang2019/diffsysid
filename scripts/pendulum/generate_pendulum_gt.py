...
def parse_args(argv=None):
    ...
    p = argparse.ArgumentParser(description="Generate and save a pendulum ground-truth trajectory for later sysID runs.")
...
def resolve_generator_request(args):
    ...
    if len(gt_values) == 1 and gt_values[0] == "random":
        centers = np.asarray([display_param(name, fixed[name], args.angle_mode) for name in fit_params], dtype=np.float64)
...
def main():
    ...
    save_ground_truth_json(
        ...
        display_value_fn=display_param,
    )

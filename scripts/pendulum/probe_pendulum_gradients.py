...
def evaluate_probe(param_name: str, gt_value: float, init_value: float, steps: int = 120, dt: float = 1.0 / 240.0):
    gt_kwargs = dict(init_angle=0.2, init_angvel=0.0)
...
def main():
    output_dir = REPO_ROOT / "outputs" / "newton_pendulum_probe"
    output_dir.mkdir(parents=True, exist_ok=True)

    probes = [
        ("init_angle", 0.20, 0.65),
        ("init_angvel", 0.00, -1.00),
    ]
    ...
    out = {
        ...
        "viability": {
            ...
            "likely_viable_for_simple_state_sysid": any(r["param"] == "init_angle" and r["grad"] is not None and abs(r["grad"]) > 1e-10 for r in results),
        },
    }

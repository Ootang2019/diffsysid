...
def maybe_prepare_worst_init_result(result_json: Path) -> Path:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="pendulum_batch_compare_"))
...
def main():
    p = argparse.ArgumentParser(description="Render gt/init/fit pendulum replays and stitch them into one comparison gif/mp4.")
    p.add_argument("--result-json", type=Path, required=True)
    p.add_argument("--output-dir", type=Path, required=True)
    p.add_argument("--label", type=str, default="pendulum_compare")
    ...
    render_script = Path(__file__).with_name("render_newton_pendulum.py")
    ...
    for variant, view_dir in view_dirs.items():
        run(
            [
                ...
                variant,
                "--subtitle",
                args.subtitle or f"pendulum {variant} replay",
                "--output-dir",
                str(view_dir),
                "--width",
                str(args.width),

...
def default_result_json(root: Path) -> Path:
    path = root / "outputs" / "newton_double_pendulum_sysid" / "result.json"
    if path.exists():
        return path
    raise FileNotFoundError(f"No double_pendulum result json found at {path}")


def main():
    p = argparse.ArgumentParser(description="Generate double-pendulum summary artifacts and gt/init/fit compare renders beside a sysID result.")
